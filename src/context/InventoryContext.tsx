...
export function InventoryProvider({ children }: { children: ReactNode }) {
  const { user, name, tenantId } = useAuth()
  ...
  const load = useCallback(async () => {
    ...
    const productQuery = client.from('products').select('*').order('code')
    const movementQuery = client
      .from('movements')
    ...
    const [{ data: productRows, error: productError }, { data: movementRows, error: movementError }] =
      await Promise.all([
        tenantId ? productQuery.eq('tenant_id', tenantId) : productQuery.eq('user_id', user.id),
        tenantId ? movementQuery.eq('tenant_id', tenantId) : movementQuery.eq('user_id', user.id),
      ])
    ...
    if (nextProducts.length === 0) {
      ...
      if (recovered.products.length > 0) {
        ...
        const reloaded = await Promise.all([
          tenantId
            ? client.from('products').select('*').eq('tenant_id', tenantId).order('code')
  ...
  }, [products, movements, loading, error, load, persistLocal, name, user, tenantId])
