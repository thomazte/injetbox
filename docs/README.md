...
InjetBox é um controle de estoque para oficina: cadastro de peças, entrada e saída, alerta de estoque baixo e histórico. Roda no navegador, no Windows (Electron) e no Android (Capacitor). O backend é o Supabase (auth + Postgres). Cada conta pertence a um tenant (estabelecimento) e o isolamento acontece por tenant.
...
O projeto roda com 1 codebase e 3 modos controlados por `VITE_APP_MODE`:

- `demo`: vitrine comercial com recursos de operação e exclusão bloqueada.
- `operacao`: gestão completa do estabelecimento.
- `catalogo`: somente consulta (sem cadastro, importação e movimentação).
...
- Produtos e movimentos filtrados por `tenant_id` (RLS).
...
- Estoque local em `injetbox.products.<userId>` e `injetbox.movements.<userId>`.
...
O InjetBox usa um único projeto Supabase. O isolamento é por linha (`tenant_id` + RLS), não por banco separado.
...
O arquivo cria extensão `pgcrypto`, tabelas `tenants`, `profiles`, `tenant_settings`, `products` e `movements`, políticas RLS, trigger `on_auth_user_created` (cria tenant + perfil), trigger `products_set_owner` (preenche `user_id` e `tenant_id` no insert), função `register_movement` e realtime em `products` e `movements`.

### Tabelas

**tenants** — estabelecimento.

**profiles** — `id` = `auth.users.id`, `tenant_id`, `name`.

**tenant_settings** — marca e cores por estabelecimento.
...
- Produtos: CRUD só quando `tenant_id = current_tenant_id()`.
- Movimentos: select e insert só no tenant do usuário. Update/delete de movimento não entram nas policies.
...
Se o schema antigo já estava aplicado, rode [`supabase/migrate-b2b-tenants.sql`](../supabase/migrate-b2b-tenants.sql) e depois o `schema.sql` de novo (policies, triggers e funções).
...
1. Job **android** (Ubuntu + Java 21): roda matriz `demo`, `operacao`, `catalogo`.
2. Job **desktop** (Windows): roda matriz `demo`, `operacao`, `catalogo`.
3. Job **publish**: anexa os APKs e `.exe` de todos os modos à GitHub Release da tag.
...
4. Se o schema antigo ainda existir, rode `supabase/migrate-b2b-tenants.sql` e depois o `schema.sql` de novo.
...
Guia de reset de histórico + estratégia de branches: [`docs/B2B-BOOTSTRAP.md`](B2B-BOOTSTRAP.md).
