  ...
  return (
    <div className="overlay fixed inset-0 z-40 lg:flex lg:items-center lg:justify-center lg:p-8" onClick={onClose}>
      ...
      >
        ...
        {!appFeatures.canMoveStock && !appFeatures.canEditProducts && !appFeatures.canDeleteProducts ? (
          <div className="mt-5 rounded-2xl bg-white/5 p-3.5 text-sm text-muted">
            Modo catálogo: apenas consulta.
          </div>
        ) : editing ? (
          ...
          <form className="mt-5 space-y-3.5" onSubmit={onMove}>
            {appFeatures.canMoveStock ? (
              ...
              <div className="rounded-2xl bg-white/5 p-3.5 text-sm text-muted">
                Movimentação desativada para este modo.
              </div>
            )}
            {error && <p className="text-sm text-danger">{error}</p>}
            {appFeatures.canMoveStock && (
            ...
            {(appFeatures.canEditProducts || appFeatures.canDeleteProducts) && (
